...
class DialogReporteInventario(tk.Toplevel):
    ...
    def _cargar_datos(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT inv_IDauto, inv_codigo, inv_descripcion, 
                       inv_categoria, inv_status, inv_unidad
                FROM ark_inventario
                ORDER BY inv_codigo
            """)
            ...
            for row in rows:
                ...
                self.tree.insert("", "end", values=(
                    row[0], row[1], row[2], row[3], status_text, row[5]
                ))
                total_registros += 1
            conn.close()
        except Exception as e:
            print(f"Error al cargar reporte de inventario: {e}")
